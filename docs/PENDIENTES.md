...
- [ ] **Campaña `windsor_nuevo_ciclo` — EN CURSO, día 2 de 6 completado (2026-09-05):** template aprobado por Meta, **150/~2,134 enviados** (50 día 1 + 100 día 2), 0 fallos de envío. Reactiva a TODOS los leads con WhatsApp (texto genérico a propósito), excluyendo `inscrito`/`perdido`/`cerrado` (`cerrado` = nombre legacy de `inscrito`, ver `LEGACY_STAGE_MAP` en `app/crm.jsx`) y RR.PP./Mercadotecnia (ya contactados con `lic_mkt`). Texto: "Hola {{1}} 👋 Se acerca el nuevo ciclo escolar en Instituto Windsor y tenemos promociones vigentes para ti. Por favor, selecciona una opción." Botones: "Me quiero inscribir" / "Tengo dudas" / "Quizás más adelante" / "Ya no me interesa" (Meta sí permite >3 Quick Reply tipo Custom). Auto-clasificación en el webhook (buscar "windsor_nuevo_ciclo" y "seguimiento_general"): → `inscripcion_pendiente` / `tercer_contacto` / `archivado` / `perdido` respectivamente. Al enviar, el script mueve el lead a `promocion_enviada`.
  - **Ritmo acordado — conservador, exponencial:** Día 1: 50 ✅ | Día 2: 100 ✅ | Día 3: 200 | Día 4: 400 | Día 5: 800 | Día 6: resto. Comando: `node --env-file=.env.local scripts/campana-ciclo-escolar.mjs --send --limit=200` (subir el número cada día).
  - **Resultados acumulados tras día 1+2 (medido 2026-09-05, ~9-24h de ventana):** 19/150 leads respondieron algo (**12.7%**). De las respuestas: texto libre (no botón) 35 mensajes (**71.4%**), "Me quiero inscribir" 7 (**14.3%**), "Tengo dudas" 7 (**14.3%**), 0 en "Quizás más adelante" y 0 en "Ya no me interesa". Stages resultantes: 5 `inscripcion_pendiente`, 3 ya avanzaron a `inscrito`/`cerrado`, 9 cayeron en `interesado` (vía clasificador de IA general, no el interceptor de botones — la mayoría de la gente prefiere escribir su pregunta directo en vez de tocar los botones del template).
  - **Próximo paso al retomar:** lanzar el lote de 200 (día 3) si no hay nuevos bugs marcados con 🚩, y seguir midiendo tasa de respuesta.
...
*Última actualización (2026-09-07): 5 bugs del bot corregidos pero SIN DESPLEGAR (verificados con tsc/eslint, falta commit/push y prueba real). Análisis de proyección de inscritos + plan de inversión con datos reales de Meta Ads (ver detalle en PROYECTOS.md). Pendientes abiertos: aclarar los "otros inscritos" fuera del CRM, decidir formato del sistema de pronóstico rodante, campaña `windsor_nuevo_ciclo` en curso, mensaje de corrección a Tania Itzel.*
